...
export default function RoleList({ roles, onEdit, onDelete }: RoleListProps) {
    ...
    const getRoleBadgeColor = (code: string) => {
        ...
        if (['TCR', 'TEACHER', 'GURU'].includes(c)) return 'bg-blue-100 text-blue-700 border-blue-200'
        if (['STD', 'STUDENT', 'MURID'].includes(c)) return 'bg-green-100 text-green-700 border-green-200'
